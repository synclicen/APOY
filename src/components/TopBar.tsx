...
export function TopBar({ currentView, onViewChange }: TopBarProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-16 bg-surface/80 backdrop-blur-md border-b border-outline-variant/10 z-50 px-8 flex items-center justify-between">
      <div className="flex items-center gap-8">
        <span className="text-xl font-black tracking-tight text-primary font-headline cursor-pointer" onClick={() => onViewChange('import')}>
          APOY
        </span>
      ...
      <div className="flex items-center gap-4">
        ...
        <button className="p-2 text-on-surface-variant hover:text-on-surface transition-colors relative">
          <Bell size={20} />
