...
export function ExportView({ photos, settings, onSettingsChange }: ExportViewProps) {
  ...
  // Export history (persisted to Turso)
  ...
  // Load export history from Turso on mount
  useEffect(() => {
    fetch('/api/history')
      .then((r) => r.json())
      .then((data) => {
    ...
    return new Promise<Blob | void>((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        const getInset = () => {
          ...
          if (selectedFrame === 'custom') return canvas.width * 0.08;
          return 0;
        };
        const inset = getInset();
        const innerWidth = canvas.width - inset * 2;
        const innerHeight =
          canvas.height - (selectedFrame === 'cinema' ? canvas.height * 0.3 : inset * 2);
        const startX = inset;
        ...
        function finalizeCanvas() {
          const qualityVal =
            selectedQuality === 'Ultra' ? 1.0 : selectedQuality === 'High' ? 0.9 : selectedQuality === 'Medium' ? 0.7 : 0.5;
  ...
  const handleExport = async () => {
    ...
    setIsExported(true);
    // Record this export to Turso-backed history
    recordExport();
    setTimeout(() => {
      setIsExported(false);
      setProgress(0);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden animate-in fade-in duration-500">
      <div className="grid grid-cols-12 gap-4 md:gap-6 flex-1 min-h-0">
        {/* Canvas Left */}
        <div className="col-span-12 lg:col-span-7 min-h-0 flex flex-col gap-4">
          <div className="relative group flex-1 min-h-0">
            {/* Glow effect */}
            <div className="absolute -inset-4 bg-gradient-to-r from-primary/10 to-secondary/10 blur-2xl opacity-50 pointer-events-none" />

            <div
              className={cn(
                'relative h-full w-full bg-surface-container-low rounded-2xl overflow-hidden shadow-2xl border border-white/5',
              )}
              ...
              >
                ...
                >
                  <div
                    ...
                    style={{ width: sliderPos > 0 ? `${100 * (100 / sliderPos)}%` : '100%' }}
                  >
                    {settings.objectFit === 'contain' && (
                    ...
                    <img
                      ...
                      style={{
                        ...
                        mixBlendMode: selectedPlatform === 'li' ? 'luminosity' : 'normal',
                ...
                {selectedFrame === 'minimal' && (
                  <div className="absolute inset-4 md:inset-8 border border-white/50 z-20 pointer-events-none" />
                )}
          ...
          <div
            className={cn(
              'shrink-0 bg-surface-container-high/40 rounded-2xl p-4 border border-primary/10 backdrop-blur-md flex flex-col sm:flex-row items-center justify-between gap-4 transition-all duration-500',
              isExported ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none hidden'
            )}
          >
        ...
        {/* Control Panel Right */}
        <div className="col-span-12 lg:col-span-5 min-h-0 flex flex-col gap-3 overflow-y-auto custom-scrollbar pr-1">
          {/* Automatic Frames */}
          <section className="bg-surface-container-low rounded-2xl p-4 md:p-5 border border-white/5 shrink-0">
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center gap-4">
                <h2 className="font-headline text-lg font-bold">Automatic Frames</h2>
                <button
                  onClick={() => {
                    if (selectedFrame !== 'none') {
              ...
              <span className="text-[10px] font-label font-bold uppercase tracking-widest text-primary">Pro Presets</span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {[
                ...
                >
                  ...
                  <span className="text-[10px] font-label font-bold uppercase tracking-tighter">{f.label}</span>
                </button>
              ))}
                ...
                <div className="flex gap-3 overflow-x-auto pb-4 custom-scrollbar">
                  {settings.frameHistory.map((url, idx) => (
                    <button
                      key={idx}
                      onClick={() =>
                        onSettingsChange({ ...settings, selectedFrame: 'custom', customFrameUrl: url })
                      }
                      className={cn(
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      <span className="text-[10px] font-bold text-on-surface-variant/70">
                        Size ({settings.watermarkSize}px)
                      </span>
                      <input
                        type="range"
                        min="12"
                        ...
                        onChange={(e) =>
                          onSettingsChange({ ...settings, watermarkSize: parseInt(e.target.value) })
                        }
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          onSettingsChange({ ...settings, watermarkOpacity: parseInt(e.target.value) })
                        }
                        className="w-full accent-primary"
                      />
                    </div>
                        ...
                        {[
                          ...
                          <button
                            ...
                            onClick={() =>
                              onSettingsChange({
                                ...settings,
                                watermarkPosition: pos.id as AnalysisSettings['watermarkPosition'],
                              })
                            }
                            className={cn(
          ...
          {/* Format & Quality */}
          <section className="bg-surface-container-low rounded-2xl p-4 md:p-5 border border-white/5 shrink-0">
            <h2 className="font-headline text-lg font-bold mb-4">Format &amp; Quality</h2>
          ...
          <section className="bg-surface-container-low rounded-2xl p-4 md:p-5 border border-white/5 shrink-0">
            <h2 className="font-headline text-lg font-bold mb-4">Business Use-case</h2>
              ...
              {[
                ...
                >
                  <div className="flex items-center gap-5">
                    <opt.icon
                      className={cn(optimization === opt.id ? 'text-primary' : 'text-on-surface-variant')}
                      size={24}
                    />
                    <div>
          ...
          {/* File Naming */}
          <section className="bg-surface-container-low rounded-2xl p-4 md:p-5 border border-white/5 shrink-0">
            <h2 className="font-headline text-lg font-bold mb-4 text-on-surface">File Naming</h2>
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-label font-bold uppercase tracking-widest text-on-surface-variant">
                  Project / Batch Name
                </label>
                <input
                  type="text"
              ...
              <div className="p-3 bg-surface-container-highest/10 rounded-xl border border-white/5">
                <p className="text-[10px] text-on-surface-variant italic">
                  Example: {exportName}
                  {selectedPhotos.length > 1 ? '-1' : ''}.{selectedFormat.toLowerCase()}
                </p>
              </div>
            </div>
          </section>
          ...
          {/* Export Destination */}
          <section className="bg-surface-container-low rounded-2xl p-4 md:p-5 border border-white/5 shrink-0">
            <h2 className="font-headline text-lg font-bold mb-4 text-on-surface">Export Destination</h2>
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button
                    key={dest.id}
                    onClick={() => setDestination(dest.id as 'local' | 'internal' | 'cloud')}
                    className={cn(
                ...
                <div className="space-y-4 pt-2">
                  ...
                  >
                    ...
                    <p className="text-[10px] text-on-surface-variant leading-relaxed">
                      {dirHandle ? (
                        <>
                          <span className="text-green-400 font-bold uppercase tracking-tight">
                            Folder Selected:
                          </span>{' '}
                        ...
                        <>
                          ...
                          Processed photos will be downloaded through your browser&apos;s default download
                          manager.
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <p className="text-[9px] text-on-surface-variant/60 px-1 italic">
                      {dirHandle ? (
                        ...
                        <>
                          Output will be saved to your device as{' '}
          ...
          <button
            ...
            className={cn(
              'shrink-0 w-full py-5 bg-gradient-to-br from-primary to-primary-container text-on-primary-container rounded-2xl shadow-2xl flex flex-col items-center justify-center gap-1 group hover:opacity-90 transition-all active:scale-[0.98] disabled:opacity-50',
          ...
          >
            ...
            <span className="font-headline font-black uppercase tracking-widest text-xs text-center">
              {isExporting
                ? 'Exporting...'
                : isExported
                  ? 'Photos Exported!'
                  : `Export ${selectedPhotos.length} ${selectedPhotos.length === 1 ? 'Photo' : 'Photos'}`}
            </span>
          </button>

          {/* Export History (persisted in Turso) */}
          <section className="bg-surface-container-low rounded-2xl border border-white/5 shrink-0 overflow-hidden">
            <button
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <h2 className="font-headline text-base font-bold">Export History</h2>
                  <p className="text-[10px] text-on-surface-variant">
                    {history.length} export{history.length === 1 ? '' : 's'} recorded
                  </p>
                </div>
              </div>
              <motion.div animate={{ rotate: showHistory ? 180 : 0 }} className="text-on-surface-variant">
                <ChevronDown size={16} />
              </motion.div>
            </button>
            <AnimatePresence>
              {showHistory && (
                ...
                >
                  <div className="max-h-48 overflow-y-auto custom-scrollbar">
                    {history.length === 0 ? (
                      <div className="p-6 text-center text-[11px] text-on-surface-variant/50">
                        No exports yet — your exports will be logged here.
                      </div>
                    ) : (
                      history.map((h) => (
                        <div
                          key={h.id}
                          className="flex items-center gap-3 p-3 border-b border-white/5 last:border-0 hover:bg-surface-container-high/30"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[11px] font-bold text-on-surface">
                              {h.format} • {h.quality} • {h.photo_count} photo{h.photo_count === 1 ? '' : 's'}
                            </p>
                  ...
                  <div className="p-2 bg-surface-container/50 border-t border-white/5">
            ...
            <div className="w-full max-w-md space-y-8 text-center p-8 rounded-3xl bg-surface-container border border-white/5 shadow-2xl">
              <div className="relative w-24 h-24 mx-auto">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-lg font-headline font-black text-on-surface">{Math.round(progress)}%</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-center gap-2 text-on-surface-variant text-sm">
                  <span>
                    Processing photo {currentProcessingIndex + 1} of {selectedPhotos.length}
                  </span>
                </div>
              </div>
